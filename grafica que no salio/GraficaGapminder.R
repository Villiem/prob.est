...
ggplot(gapminder2007) + aes(x = gdpPercap, y = lifeExp, color = continent, size = pop) + geom_point() + labs(title = "A mayor ingreso mayor esperanza de vida", subtitle = "Para la clase de estadistica", x = "Pib per capita", y = "Esperanza de vida") + theme_classic() + guides (size = FALSE) + scale_x_log10(breaks = c(400,4000,25000)) + transition_time(year)
ggplot(gapminder2007) + aes(x = gdpPercap, y = lifeExp, color = continent, size = pop) + geom_point() + labs(title = "A mayor ingreso mayor esperanza de vida", subtitle = "Para la clase de estadistica", x = "Pib per capita", y = "Esperanza de vida") + theme_classic() + guides (size = FALSE) + scale_x_log10(breaks = c(400,4000,25000)) + transition_time(year) + ease_aes('linear')
animado<-ggplot(gapminder2007) + aes(x = gdpPercap, y = lifeExp, color = continent, size = pop) + geom_point() + labs(title = "A mayor ingreso mayor esperanza de vida", subtitle = "Para la clase de estadistica", x = "Pib per capita", y = "Esperanza de vida") + theme_classic() + guides (size = FALSE) + scale_x_log10(breaks = c(400,4000,25000)) + transition_time(year) + ease_aes('linear')
animado
